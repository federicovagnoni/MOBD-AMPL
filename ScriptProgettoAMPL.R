...
data_por_final <- mutate(data_por_final, GP = ifelse(school == "GP", 1, 0))
...
data_por_final <- mutate(data_por_final, Pstatus_O = ifelse(Pstatus == "O", 1, 0))
...
write.table(train, quote= FALSE, row.names = FALSE, col.names = FALSE, file="/home/federico/Scrivania/training_por.txt")
write.table(val, quote= FALSE, row.names = FALSE, col.names = FALSE, file="/home/federico/Scrivania/validation_por.txt")
